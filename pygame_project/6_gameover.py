...
total_time = 100
start_ticks = pygame.time.get_ticks() # set start time 

# Message when the game is finished 
# "Time Out"
# "Mission Complete"
# "Game Over"
...
while running:
    ...
    for ball_index, ball_value in enumerate(balls) : 
        ...
        for weapon_index, weapon_value in enumerate(weapons) : 
            ...
            if weapon_rect.colliderect(ball_rect) :
                ...
                # if this is the smallest ball then the ball will be divided
                if ball_img_index < 3 :
                    # get the current ball's info
                    ball_width = ball_rect.size[0]
                    ball_height = ball_rect.size[0]

                    # get the divided ball's info
                    small_ball_rect = ball_images[ball_img_index + 1].get_rect()
                    small_ball_width = small_ball_rect.size[0]
                    small_ball_height = small_ball_rect.size[1]

                    # to left
                    balls.append({ "pos_x" : ball_pos_x + (ball_width / 2) - (small_ball_width / 2),  # x position of the ball
                                   "pos_y" : ball_pos_y + (ball_height / 2) - (small_ball_height / 2),  # y position of the ball
                                   "img_index" : ball_img_index + 1, # index of the ball
                                   "to_x" : -3,    # movement to x, -3 to left, 3 to right
                                   "to_y" : -6,   # movement to y
    ...
    # when all the balls are removed
    if len(balls) == 0 :
        game_result = "MISSOION COMPLETE"
        running = False
